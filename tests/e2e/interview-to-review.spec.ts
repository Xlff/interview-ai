...
test("anonymous user can start a focused retry interview from review", async function ({
  page,
}) {
  await page.goto("/");
  ...
  const weakAnswer = "我做过一些项目，主要是配合同事完成开发。";

  for (let round = 0; round < 6; round += 1) {
    await page.getByRole("textbox", { name: "你的回答" }).fill(weakAnswer);
    await page.getByRole("button", { name: "提交回答，进入下一题" }).click();
  ...
  await page.getByRole("button", { name: "围绕短板再练一轮" }).click();
  await page.waitForURL(/\/interview\/.+/);
  await expect(page.getByText("聚焦短板强化")).toBeVisible();
});
